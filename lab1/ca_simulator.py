...
class CASimulator():
    ...
    def calculate_Zeta(self):
        ...
        # https://stackoverflow.com/questions/44589054/compression-ratio-for-numpy-array

        compressed = io.BytesIO()    # np.savez_compressed() requires a file-like object to write to
        uncompressed = io.BytesIO()
        np.savez_compressed(compressed, self.board)
        np.savez(uncompressed, self.board)
        
        return len(compressed.getvalue())/len(uncompressed.getvalue())
